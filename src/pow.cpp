...
unsigned int CalculateNextTargetRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params, bool fProofOfStake)
{
    ...
    int64_t nTargetSpacing = params.IsProtocolBATL(pindexLast->GetBlockTime()) ? params.nTargetSpacingBATL : ( params.IsProtocolV2(pindexLast->GetBlockTime()) ? params.nTargetSpacing : params.nTargetSpacingV1 );
