...
static int64_t GetStakeCombineThreshold() { return GetArg("-stakesplit", 100) * COIN; } // WAS 500
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CAmount& nFees, CMutableTransaction& tx, CKey& key)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(const CWalletTx*, unsigned int)& pcoin, setCoins)
    {
            ...
            && pcoin.first->GetHash() != txNew.vin[0].prevout.hash)
        {
            ...
            if (txNew.vin.size() >= GetArg("-stakemaxcombine", 10))
                break;
    ...
    // Calculate reward
    {
        int64_t nReward = nFees + GetProofOfStakeSubsidy(pindexPrev->nTime);
        if (nReward < 0)
           return false;
